...
SetupCraft = function()
    ...
    if Shared.Framework == "qb" or Shared.Framework == "qbox" then
    ...
    if formData then
        ...
        if (Shared.Framework == "qb" or Shared.Framework == "qbox") and formData['input-5'] == true then
...
local CanAcces = function(id)
    ...
    if (Shared.Framework == "qb" or Shared.Framework == "qbox") and gangs ~= nil and #gangs > 0 then
        for i = 1, #gangs do
...
function createRecipe(numRecipe, cb)
    ...
    for i = 1, numRecipe do
        ...
        local recipeInput = QT.CreateBox(L('reciepe_item').. ' (' .. i .. '/' ..numRecipe..')', {
            ...
            { label = L("removable"), value = 'checkbox2', type = 'checkbox' },
        })
        if not recipeInput then return end 
         table.insert(recipetable, { 
            item = recipeInput['input-1'], 
            label = recipeInput['input-2'], 
            amount = tonumber(recipeInput['input-3']), 
            removable = recipeInput['input-4']
        })
    end
    cb(recipetable)
end
...
AddEventHandler("qt_crafting-ItemUpdateMenu2", function(data)
    ...
    if data.action == "ChangeLabelItem" then 
        ...
        if formData then 
            TriggerServerEvent("qt_crafting-EditActions", { action = data.action, new_label = formData['input-1'], id = data.id, item = data.item, label = data.label })
        end
    elseif data.action == "ChangeCraftTime" then 
        ...
        if formData then 
            TriggerServerEvent("qt_crafting-EditActions", { action = data.action, new_time = formData['input-1'], id = data.id, item = data.item, label = data.label })
        end
    elseif data.action == "ChangeRecipeItem" then 
        ...
        if formData then 
            createRecipe(formData['input-1'], function(recipe)
               TriggerServerEvent("qt_crafting-EditActions", { action = data.action, item = data.item, id = data.id, recipe = recipe, label = data.label })
            end)
        end
    elseif data.action == "ChangeReward" then 
        ...
        if formData then 
            TriggerServerEvent("qt_crafting-EditActions", { action = data.action, amount = tonumber(formData['input-1']), id = data.id, item = data.item, label = data.label })
        end
    end
    String.SendAlert({
        type = "success",
